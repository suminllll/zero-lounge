...
export default function AdminPage() {
  ...
  const addEvent = async () => {
    ...
    if (!error) {
      setNewEvent({ date: '', time: '20:00', female_seats: 6, male_seats: 6 })
      fetchEvents()
    }
  }
  ...
  const toggleParty = async (partyType: string, current: boolean) => {
    ...
    setPartySettings(prev =>
      prev.map(p => (p.party_type === partyType ? { ...p, is_visible: !current } : p))
    )
  }
  ...
  if (!isAuthed) {
    return (
      <main className="min-h-screen bg-[#1a1210] flex items-center justify-center px-6">
        <div className="w-full max-w-sm">
          <p className="text-center text-[#c6beb8] text-2xl font-bold tracking-widest mb-10">
            ZERO LOUNGE
          </p>
          <p className="text-center text-[#8F8781] text-sm mb-6">관리자 페이지</p>
          <input
            type="password"
            value={password}
          ...
          <button
            onClick={handleLogin}
            className="block w-full py-4 rounded-2xl text-center font-bold text-[#1a1210] text-[17px]"
  ...
  const DetailModal = ({ app }: { app: Application }) => (
    ...
    >
      <div
        className="bg-[#1a1210] w-full max-w-lg rounded-t-3xl p-6 pb-10 max-h-[85vh] overflow-y-auto"
        onClick={e => e.stopPropagation()}
      >
        <div className="flex justify-between items-center mb-6">
          <p className="text-[#f5e2d4] font-bold text-lg">{app.name} ({app.nickname})</p>
          <button onClick={() => setSelectedApp(null)} className="text-[#8F8781] text-2xl leading-none">×</button>
        </div>
        {app.photo_url && (
          <img src={app.photo_url} alt="신청자 사진" className="w-full h-48 object-cover rounded-2xl mb-5" />
        )}
        <div className="flex flex-col gap-3 text-sm">
          {[
            ...
            <div key={label} className="flex gap-3">
              <span className="text-[#8F8781] w-20 shrink-0">{label}</span>
              <span className="text-[#f5e2d4]">{value}</span>
            </div>
          ))}
        </div>
        <div className="mt-6 flex gap-3">
          {(['confirmed', 'rejected', 'pending'] as const).map(s => (
            <button
              ...
              className={`flex-1 py-3 rounded-2xl text-sm font-bold ${s === 'rejected' ? 'text-white' : 'text-[#1a1210]'}`}
  ...
  // ══════════════════════════════════════════
  return (
    <main className="min-h-screen bg-[#1a1210] text-[#f5e2d4] pb-24">
      {selectedApp && <DetailModal app={selectedApp} />}

      {/* 헤더 */}
      <div className="sticky top-0 bg-[#1a1210] z-10 px-5 py-4 border-b border-[#2a2220]">
        <p className="text-[#c6beb8] font-bold text-lg tracking-widest">ADMIN</p>
      </div>
      ...
      <div className="pb-4">
          ...
          <>
            ...
            <div className="px-5 py-4 flex gap-2 overflow-x-auto">
              <input
                ...
                className="bg-[#2a2220] rounded-xl px-3 py-2 text-[#c6beb8] text-sm outline-none shrink-0"
              />
              <select
                value={filterGender}
              ...
              <button
                ...
                className="bg-[#c6beb8] text-[#1a1210] rounded-xl px-4 py-2 text-sm font-bold shrink-0"
              >
                조회
              </button>
            </div>
            ...
            <div className="px-5 mb-4 grid grid-cols-3 gap-2">
              {(['pending', 'confirmed', 'rejected'] as const).map(s => (
                <div key={s} className="bg-[#2a2220] rounded-2xl p-4 text-center">
            ...
            {appsLoading ? (
              ...
              <p className="text-center text-[#8F8781] text-sm pt-16">신청 내역이 없습니다.</p>
            ) : (
              <div className="px-5 flex flex-col gap-3">
                {applications.map(app => (
                  <button
                    ...
                    className="w-full bg-[#2a2220] rounded-2xl p-4 text-left flex items-center gap-4"
                  >
                    {app.photo_url ? (
                      <img src={app.photo_url} alt="" className="w-12 h-12 rounded-full object-cover shrink-0" />
                    ) : (
                      <div className="w-12 h-12 rounded-full bg-[#3a3230] flex items-center justify-center shrink-0">
                        <span className="text-[#8F8781] text-lg">{app.name.charAt(0)}</span>
                      </div>
                    )}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-1">
                        <p className="font-bold text-[#f5e2d4] truncate">
                      ...
                      <p className="text-[#8F8781] text-xs truncate">{app.contact}</p>
                    </div>
                  </button>
                ))}
              </div>
        ...
        {tab === 'events' && (
          <div className="px-5 pt-4">
            {/* 이벤트 추가 폼 */}
            <div className="bg-[#2a2220] rounded-2xl p-5 mb-5">
              <p className="text-[#c6beb8] font-bold text-sm mb-4">새 이벤트 추가</p>
              <div className="flex flex-col gap-3">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-[#1a1210] rounded-xl px-3 py-2.5 text-[#f5e2d4] text-sm outline-none"
                  />
                  <input
                    type="time"
                    value={newEvent.time}
                    onChange={e => setNewEvent(p => ({ ...p, time: e.target.value }))}
                    className="w-28 bg-[#1a1210] rounded-xl px-3 py-2.5 text-[#f5e2d4] text-sm outline-none"
                  />
                </div>
                <div className="flex gap-2">
                  <div className="flex-1 bg-[#1a1210] rounded-xl px-3 py-2.5 flex items-center gap-2">
                    <span className="text-[#8F8781] text-xs shrink-0">여성</span>
                    <input
                      type="number"
                      value={newEvent.female_seats}
                      onChange={e => setNewEvent(p => ({ ...p, female_seats: Number(e.target.value) }))}
                      className="flex-1 bg-transparent text-[#f5e2d4] text-sm outline-none w-0"
                    />
                  </div>
                  <div className="flex-1 bg-[#1a1210] rounded-xl px-3 py-2.5 flex items-center gap-2">
                    <span className="text-[#8F8781] text-xs shrink-0">남성</span>
                    <input
                      type="number"
                ...
                <button
                  ...
                  className="w-full py-3 rounded-xl text-sm font-bold text-[#1a1210] disabled:opacity-40"
                  style={{ backgroundColor: '#c6beb8' }}
                >
                  추가
                </button>
                  ...
                  return (
                    <div key={ev.id} className="bg-[#2a2220] rounded-2xl p-4">
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          <p className="text-[#f5e2d4] font-bold">{ev.date}</p>
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className="text-xs px-3 py-1.5 rounded-lg bg-[#3a3230] text-[#c6beb8]"
                          >
                            {isEditing ? '취소' : '수정'}
                          </button>
                          <button
                            onClick={() => deleteEvent(ev.id)}
                            className="text-xs px-3 py-1.5 rounded-lg bg-[#3a3230] text-[#f87171]"
                          >
                            삭제
                          </button>
                        </div>
                      ...
                      {isEditing ? (
                        <div className="flex flex-col gap-2 mt-2">
                          <div className="flex gap-2">
                            <input
                              ...
                              className="flex-1 bg-[#1a1210] rounded-xl px-3 py-2 text-[#f5e2d4] text-sm outline-none"
                            />
                          </div>
                          <div className="flex gap-2">
                            <div className="flex-1 bg-[#1a1210] rounded-xl px-3 py-2 flex items-center gap-2">
                              <span className="text-[#8F8781] text-xs shrink-0">여성</span>
                              <input
                                type="number"
                          ...
                          <button
                            ...
                            className="w-full py-2.5 rounded-xl text-sm font-bold text-[#1a1210]"
                            style={{ backgroundColor: '#c6beb8' }}
                          >
                            저장
                          </button>
                        ...
                        <div className="flex gap-3 mt-1">
                          <div className="flex-1 bg-[#1a1210] rounded-xl p-3 text-center">
                            <p className="text-[#8F8781] text-xs mb-1">여성 잔여</p>
                            <p className="text-[#f5e2d4] text-xl font-bold">{display.female}</p>
        ...
        {tab === 'parties' && (
          <div className="px-5 pt-4">
            <p className="text-[#8F8781] text-xs mb-5">
                ...
                {[
                  ...
                  return (
                    <div key={type} className="bg-[#2a2220] rounded-2xl p-5 flex items-center justify-between">
                      <div>
                        <p className="text-[#f5e2d4] font-bold">{label}</p>
      ...
      {/* 하단 탭 바 */}
      <div className="fixed bottom-0 left-0 right-0 bg-[#1a1210] border-t border-[#2a2220] flex z-20">
        {([
          ...
          <button
            ...
            className={`flex-1 py-4 text-sm font-bold transition-colors ${
              tab === key ? 'text-[#c6beb8]' : 'text-[#4a3e3a]'
            }`}
          >
            {label}
          </button>
